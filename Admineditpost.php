...
if (isset($_POST['submit']) && isset($_FILES['image'])) {
    ...
    $time = date('h:i A');
    $date = date('d/m/y');
    $query2 = "UPDATE Posts SET PostImagePath = '$image_path', PostTime = '$time', PostDate = '$date' WHERE PostId = '$posted_id'";
    mysqli_query($con, $query2);
    header('Location: Adminhome.php');
    echo "<div class='alert alert-success'> Post is updated successfully! </div>";
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
        ...
        <nav class="navbar navbar-expand-lg bg-light" id="navbar-border">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" href="Admineditpost.php">Edit Post</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" aria-current="page" href="Adminaddblog.php">Add Blog</a>
                    </li>
                </ul>
    ...
    <section id="contact">
        <div class="container custom-size-laptop-container custom-size-mobile-container">
            <h3 class="text-center mt-5 mb-5 bg-danger p-2 text-white">Edit Post</h3>
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
                <div class="row">
                    <div class="col-12 text-center">
                        <?php
                        $result2 = mysqli_query($con, "SELECT * FROM `Posts` WHERE PostId = $id");
                        $row2 = mysqli_fetch_assoc($result2);
                        ?>
                        <input type="text" name="posted_id" value="<?php echo $id; ?>" hidden>
                        <img src="<?php echo $row2['PostImagePath']; ?>" alt="" width="100" height="100" style="border-radius: 50%;" id="userImage">

                    </div>
                </div>
                <div class="row">
                    <label for="" class="text-danger fs-3 mt-3 mb-3">Select the Picture*</label>
                    <div class="col-12"><input id="imageInput" type="file" class="w-100 p-2 fs-4 border border-danger custom-outline text-danger" name="image" value="<?php echo $row2['PostImagePath']; ?>">
                    </div>
                </div>
                <div class="row">
                    <div class="col-12"><input type="submit" class="w-100 p-2 fs-4 border border-none text-white" style="background-color: black; color: white; margin-top: 10px; margin-bottom: 10px;" name="submit" value='Edit Projects'>
                    </div>
                </div>
            </form>
        </div>
